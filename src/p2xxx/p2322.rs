...
impl Solution {
    pub fn minimum_score(nums: Vec<i32>, edges: Vec<Vec<i32>>) -> i32 {
        ...
        /// Find TreeInfo
        fn find_tree_info(xor_table: &mut XORTable, neighbours: &Vec<Vec<u16>>, nums: &Vec<i32>, key: TreeKey) -> Rc<TreeInfo> {
            ...
            for &child in neighbours[node].iter() {
                if child != parent {
                    let child_info = find_tree_info(xor_table, neighbours, nums, TreeKey::new(child, key.node));
                    xor ^= child_info.xor;
